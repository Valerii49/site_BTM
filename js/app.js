...
// Анимация =================================== 
/* 1. анимируются объекты с классом '._anim-items' 
	2. слушается прокрытка, событие 'scroll'
	3. При 	const animStart = 4; объект начнет появляться при прокрутке га четверть объекта, если объект больше экрана, то при прокруткена четверь экрана. 
	константа  animItemOffset - верх объекта относительно верха страницы
	переменная animItemPoint - дельта (четверть от размера объекта) на которую надо задержать запуск анимации
*/
// В переменной все объекты которые анимируются
const animItems = document.querySelectorAll('._anim-items');
// Проверка, существуют ли такие классы. Больше 0 длина масива
if (animItems.length > 0) {
	// событие 'scroll' на все окно браузера
	window.addEventListener('scroll', animOnScroll);

	function animOnScroll() {
		if (pageYOffset == 0) {
			// костыль при запуске первого экрана. '.firstscreen__par при маленькой высоте экрана не выезжает
			document.querySelector('.firstscreen__par').classList.add('_active');
			document.querySelector('.firstscreen__button-wrapper').classList.add('_active');

		}
		for (let index = 0; index < animItems.length; index++) {
			const animItem = animItems[index];
			// Высота объекта, который будет анимироваться
			const animItemHeight = animItem.offsetHeight;
			// Позиция объекта относительно верха
			const animItemOffset = offset(animItem).top;
			// коэф регулирующий момент старта анимации
			const animStart = 4;
			// от высоты экрана window.innerHeight отнимается высота объекта поделенная на коэф
			let animItemPoint = window.innerHeight - animItemHeight / animStart;
			// если высота объекта больше высоты окна браузера то делим не высоту объекта, а высоту окна на коэф
			if (animItemHeight > window.innerHeight) {
				animItemPoint = window.innerHeight - window.innerHeight / animStart;
			}
			//pageYOffset переменная с количеством проскроленных пикселей. Если прокручено больше чем позиция объекта - (минус) точка старта, но прокрутити меньше чем позиция объекта плюс его высота, то добавляем класс _active
			if ((pageYOffset > animItemOffset - animItemPoint) && pageYOffset < (animItemOffset + animItemHeight)) {
				animItem.classList.add('_active');
			} else {
				// условие не выполняется отбираем класс, чтобы могли повторно анимировать объект если не хотим повторно анимировать, в html у объекта должен стоять класс '_anim-no-hide'
	...
	// возвращает смещение объекта относительно верха страницы (можно еще получить значение слева) есть в scroll.js 
	function offset(el) {
	...
	// запуск первого экрана без скролла 300 ms
	setTimeout(() => {
		animOnScroll();
	}, 300);
}
